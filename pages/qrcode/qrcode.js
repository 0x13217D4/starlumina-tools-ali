Page({
  ...
  generateQRCode() {
    ...
    if (this.data.mode === 'text') {
      ...
      if (!text) {
        my.showToast({ title: '请输入二维码内容', duration: 2000 });
        return;
      }
      ...
      if (!ssid) {
        my.showToast({ title: '请输入WIFI名称', duration: 2000 });
        return;
      }
      this.generateWifiQR();
    }
  ...
  generateTextQR(text) {
    ...
    try {
      ...
      my.showToast({ title: '生成失败', duration: 2000 });
      this.setData({ isLoading: false });
    }
  },
  ...
  saveQRCode() {
    ...
    my.downloadFile({
      ...
      fail: () => {
        my.showToast({ title: '下载失败', duration: 2000 });
