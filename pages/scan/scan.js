Page({
  ...
  parseBoardingPass(result) {
    ...
    try {
      ...
      if (pos + 3 <= result.length) {
        const departureCode = result.substring(pos, pos + 3);
        boardingPass.departureCity = this.getAirportName(departureCode);
      }
      pos += 3;
            
      // 到达城市 (3 位): IATA 机场代码
      if (pos + 3 <= result.length) {
        const arrivalCode = result.substring(pos, pos + 3);
        boardingPass.arrivalCity = this.getAirportName(arrivalCode);
      }
      pos += 3;
  ...
  parseChineseBoardingPass(result) {
    ...
    try {
      ...
      if (routeMatch) {
        const departureCode = routeMatch[1].toUpperCase();
        const arrivalCode = routeMatch[2].toUpperCase();
        boardingPass.departureCity = this.getAirportName(departureCode);
        boardingPass.arrivalCity = this.getAirportName(arrivalCode);
      }
  ...
  startScan() {
    ...
    my.scan({
      hideAlbum: true,
      scanType: ['qrCode', 'barCode', 'dmCode', 'pdf417Code'],
      success: (res) => {
        console.log('扫码成功:', res);
        // 根据文档，返回值包含 code 和 result
        const result = res.code || res.result;
        if (result) {
      ...
      fail: (err) => {
        ...
        if (err.error === 10) {
          console.log('用户取消扫码');
        } else if (err.error === 2001 || err.error === 2002 || err.error === 2003) {
          // 授权相关错误
          my.showToast({
            content: '请授权摄像头权限',
            type: 'none'
  ...
  // 加载机场代码映射表
  loadAirportCodes: function() {
    try {
      const airportData = require('../../utils/airport-codes.js');
      if (airportData && typeof airportData === 'object') {
        this.airportCodesMap = airportData;
        console.log('机场代码映射表加载成功，共', Object.keys(airportData).length, '个机场');
      }
    } catch (e) {
      console.error('加载机场代码映射表失败:', e);
      this.airportCodesMap = {};
    }
  },
  ...
  getAirportName: function(code) {
    ...
    // 从映射表中查找
    if (this.airportCodesMap && this.airportCodesMap[airportCode]) {
      return this.airportCodesMap[airportCode];
    }
