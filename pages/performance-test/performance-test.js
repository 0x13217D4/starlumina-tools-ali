...
const performanceTestPage = {
  ...
  loadBasicInfo() {
    ...
    my.getSystemInfo({
      success: (res) => {
        // 根据平台优化显示信息
        let platformDisplay = res.platform;
        let systemDisplay = `${res.brand} ${res.model}`;
        
        if (res.platform === 'ohos') {  // 鸿蒙系统
          platformDisplay = 'HarmonyOS';
          systemDisplay = `HarmonyOS ${res.system.replace('OpenHarmonyOS ', '')}`;
        }
    ...
    for (let i = 0; i < tests.length; i++) {
      ...
      try {
        ...
        if (this.data.testing) {
          ...
          this.updateProgress((tests[i + 1] && tests[i + 1].name) || '测试完成', (i + 1) * progressPerTest);
        }
      } catch (error) {
